  ...
  for _, v in pairs(M.pandatree.tree_list) do
    if v.level > 0 then
      ...
      elseif v.t == 'link' then
        ...
        local icon, hl_group = require'utils.icons'.get_icon(v.name, extension)
        if icon == nil then
...
local function set_mappings()
  ...
  for k,v in pairs(mappings) do
    vim.api.nvim_buf_set_keymap(buf, 'n', k, ':lua require"panda.pandatree".'..v..'<cr>', {
...
function pandatree_augroup()
  ...
  vim.api.nvim_command('autocmd CursorMoved <buffer> lua require"panda.pandatree".cursor_moved()')
  vim.api.nvim_command('autocmd BufWritePost * lua require"panda.pandatree".draw_tree()')
  vim.api.nvim_command('autocmd BufEnter * lua require"panda.pandatree".sync_tab_tree()')
  vim.api.nvim_command('autocmd BufEnter * lua require"panda.pandatree".exit()')
...
local function open_tree()
  ...
  require'panda.pandaline'.load_pandaline(true)
  require'panda.pandaline'.load_tabline()
end
